<?php
 /**
  * Routes.
 */
require __DIR__ . "/route/base.php";
require __DIR__ . "/route/about.php";
require __DIR__ . "/route/report.php";
require __DIR__ . "/route/status.php";
require __DIR__ . "/route/calandar.php";
require __DIR__ . "/route/profile.php";
require __DIR__ . "/route/internal.php";
require __DIR__ . "/route/login.php";
require __DIR__ . "/route/register.php";
require __DIR__ . "/route/logout.php";
require __DIR__ . "/take2Routes/filter.php";
require __DIR__ . "/take2Routes/overview.php";
require __DIR__ . "/take2Routes/blogg.php";
require __DIR__ . "/take2Routes/pages.php";
require __DIR__ . "/take2Routes/block.php";

require __DIR__ . "/route/back.php";
require __DIR__ . "/route/sessionstatus.php";
require __DIR__ . "/route/destroy.php";
require __DIR__ . "/route/dump.php";
require __DIR__ . "/route/session.php";
require __DIR__ . "/route/forward.php";
require __DIR__ . "/route/increment.php";
require __DIR__ . "/route/decrement.php";
require __DIR__ . "/route/validate.php";
require __DIR__ . "/route/check.php";
require __DIR__ . "/route/changePassword.php";
require __DIR__ . "/route/check_change_pass.php";
require __DIR__ . "/route/admin.php";
require __DIR__ . "/route/edit.php";
