...
class Database
{
    ...
    public function connect()
    {
        try {
            ...
            $db->setAttribute(\PDO::ATTR_DEFAULT_FETCH_MODE, $this->options['fetch_mode']);
    ...
    {
        $stmt = $this->db->prepare("INSERT into users (name, password) VALUES ('$user', '$pass')");
        $stmt->execute();
    }

    public function getHash($user)
    {
        $stmt = $this->db->prepare("SELECT password FROM users WHERE name='$user'");
        $stmt->execute();
    ...
    {
        $stmt = $this->db->prepare("UPDATE users SET password='$pass' WHERE name='$user'");
        $stmt->execute();
    }
    ...
    /**
     * Get databas content from table users back as array
     * @return databas as obj
     */
    public function getAssArray()
    {
    ...
    {
        // $sql = "SELECT * FROM users;";
        $sth = $this->db->prepare("SELECT * FROM users WHERE $where='$name';");
        $this->db->setAttribute(\PDO::ATTR_DEFAULT_FETCH_MODE, \PDO::FETCH_OBJ);
        $sth->execute();
        $res = $sth->fetchAll();
        return $res;
